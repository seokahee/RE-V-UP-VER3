...
const PreviousButton = ({ children, onClick }: ButtonProps) => {
  const shadow =
  ...
  return (
    <button
      type='button'
      className={`bg-white bg-opacity-20 p-3 ${shadow} ${active} min-w-[118px] rounded-xl text-[1rem] font-bold tracking-[-0.03em]`}
      onClick={onClick}
    >
      {children}
    </button>
