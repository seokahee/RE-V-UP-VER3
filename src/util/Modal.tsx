...
const Modal = ({ isOpen, children, closeModal }: ModalProps) => {
  return (
    //모달
    <section
      className={`${isOpen ? "relative top-0 left-0 flex justify-center items-center max-w-[400px] bg-neutral-900/70 z-50" : "hidden"}`}
    >
      {isOpen && (
        <>
          <div
          ...
          <div className="bg-white rounded-lg p-8 max-w-[400px] relative z-50">
            ...
            >
              <p className="flex bg-green-400 w-full h-[10px] font-bold">X</p>
            </button>
